...
# sys.path.append('/content/drive/MyDrive/MVRNN')
...
def preprocess_data(df_data):
    ...
    if FLAGS.dataset == 'GEOSD':
        x = np.array(df_data['mean'])
        y = np.array(df_data['mean'])
    else:
...
def train(prepared_data,model,criterion,optimizer,epoch):
    model.train()
    optimizer.zero_grad()
    train_x, train_y, validate_x, validate_y, test_x, test_y = prepared_data[0], prepared_data[1], prepared_data[2], prepared_data[3], prepared_data[4], prepared_data[5]
...
def compute_test(best_model,prepared_data):
    model = best_model
    train_x, train_y, validate_x, validate_y, test_x, test_y = prepared_data[0], prepared_data[1], prepared_data[2], prepared_data[3], prepared_data[4], prepared_data[5]
    if FLAGS.algorithm in ['VRNN', 'mVRNN', 'mVRNN_fixD', 'VRNN_WAE', 'mVRNN_WAE', 'mVRNN_fixD_WAE']:
...
def run_seed(seed):
    ...
    prepared_data = preprocess_data(df_data)
    ...
    if FLAGS.algorithm == 'RNN':
        ...
        model = models.GRU(input_size=FLAGS.input_size, hidden_size=FLAGS.hidden_size, output_size=FLAGS.output_size)
    elif FLAGS.algorithm == 'AttnLSTM':
        model = models.AttnLSTM(input_size=FLAGS.input_size, hidden_size=FLAGS.hidden_size, output_size=FLAGS.output_size)
    ...
    while epoch < FLAGS.epochs:
        _time = time.time()
        if FLAGS.algorithm in ['VRNN', 'mVRNN', 'mVRNN_fixD', 'VRNN_WAE', 'mVRNN_WAE', 'mVRNN_fixD_WAE']:
            loss, val_loss, info = train(prepared_data,model,criterion,optimizer,epoch)
            if FLAGS.algorithm in ['VRNN', 'mVRNN', 'mVRNN_fixD']:
                epoch_info['kl_loss'] = np.append(epoch_info['kl_loss'], info['kl_loss'].mean(dim=0).detach().cpu().numpy())
        else:
            loss, val_loss = train(prepared_data,model,criterion,optimizer,epoch)
        ...
        if FLAGS.algorithm in ['VRNN', 'mVRNN', 'mVRNN_fixD', 'VRNN_WAE', 'mVRNN_WAE', 'mVRNN_fixD_WAE']:
            ...
            if FLAGS.algorithm in ['VRNN', 'mVRNN', 'mVRNN_fixD']:
                ...
                print("  " + "KL Loss: {:.6f}\t Prior Mean: {:.6f}\t Prior Std: {:.6f}\t Encoder Mean: {:.6f}\t Encoder Std: {:.6f}\t Decoder Mean: {:.6f}\t Decoder Std: {:.6f}".format(
                    kl_loss_t, prior_mean_t, prior_std_t, enc_mean_t, enc_std_t, dec_mean_t, dec_std_t
                    ))
            elif FLAGS.algorithm in ['VRNN_WAE', 'mVRNN_WAE', 'mVRNN_fixD_WAE']:
    ...
    # make predictions
    if FLAGS.algorithm in ['VRNN', 'mVRNN', 'mVRNN_fixD']:
        test_rmse, test_mape, test_mae, var_loss = compute_test(best_model,prepared_data)
    elif FLAGS.algorithm in ['VRNN_WAE', 'mVRNN_WAE', 'mVRNN_fixD_WAE']:
        test_rmse, test_mape, test_mae, wae_loss = compute_test(best_model,prepared_data)
    else:
        test_rmse, test_mape, test_mae = compute_test(best_model,prepared_data)
            
    elapsed_per_seed = time.time() - start_time
    if FLAGS.algorithm in ['VRNN', 'mVRNN', 'mVRNN_fixD']:
        return {'seed': seed, 'epoch': epoch, 'elapsed_per_seed': elapsed_per_seed, 'test_rmse':test_rmse, 
                'test_mae': test_mae, 'var_loss': np.mean(var_loss.detach().numpy()), 'best_loss': best_loss, 
                'best_model': {k: v.detach() for k, v in best_model.state_dict().items()}}
    elif FLAGS.algorithm in ['VRNN_WAE', 'mVRNN_WAE', 'mVRNN_fixD_WAE']:
        return {'seed': seed, 'epoch': epoch, 'elapsed_per_seed': elapsed_per_seed, 'test_rmse':test_rmse, 
                'test_mae': test_mae, 'wae_loss': np.mean(wae_loss.detach().numpy()), 'best_loss': best_loss, 
                'best_model': {k: v.detach() for k, v in best_model.state_dict().items()}}
    else:
        return {'seed': seed, 'epoch': epoch, 'elapsed_per_seed': elapsed_per_seed, 'test_rmse':test_rmse, 
                'test_mae': test_mae, 'best_loss': best_loss, 'best_model': {k: v.detach() for k, v in best_model.state_dict().items()}}
    
if __name__ == "__main__":
    mp.set_start_method('spawn', force=True)
    max_seed = 10
    seeds = range(0,max_seed)
    ...
    with mp.Pool(6) as p:
        for out in tqdm(p.map(run_seed, seeds), total=len(seeds)):
            ...
            print('RMSE:{}'.format(results[seed]['test_rmse']))
            print('MAE:{}'.format(results[seed]['test_mae']))
            if FLAGS.algorithm in ['VRNN', 'mVRNN', 'mVRNN_fixD']:
                print("KL Loss:{}".format(results[seed]['var_loss']))
            elif FLAGS.algorithm in ['VRNN_WAE', 'mVRNN_WAE', 'mVRNN_fixD_WAE']:
                print("WAE MMD Loss:{}".format(results[seed]['wae_loss']))
    ...
    rmse_list = [results[seed]['test_rmse'] for seed in results.keys()]
    mae_list = [results[seed]['test_mae'] for seed in results.keys()]
    ...
    if FLAGS.algorithm in ['VRNN', 'mVRNN', 'mVRNN_fixD']:
        var_list = [results[seed]['var_loss'] for seed in results.keys()]
        ...
        wae_list = [results[seed]['wae_loss'] for seed in results.keys()]
    ...
    PATH = f"time_series_prediction/saved_models/{FLAGS.algorithm}_{FLAGS.dataset}_{FLAGS.epochs}_{FLAGS.batch_size}_{FLAGS.lr}_{max_seed}"
    torch.save(best_model_global, PATH)
