...
def main():
    ...
    log_dir = 'results'
    log_file_name = f'{FLAGS.dataset}_{FLAGS.algorithm}_{FLAGS.K}_{FLAGS.lr}'
    os.makedirs(log_dir, exist_ok=True)
    file_path = os.path.join(log_dir, log_file_name)

    with open(file_path, 'a') as f:
        f.write(f"{FLAGS.algorithm}\n")
        f.write("MEAN ACC:{:.4f}, STD ACC:{:.4f}, MAX ACC:{:.4f}\n"
                .format(mean_acc,std_acc,max_acc))
        f.write("MEAN LOSS:{:.4f}, STD LOSS:{:.4f}, MIN LOSS:{:.4f}\n"
                .format(mean_loss,std_loss,min_loss))
        f.write("MEAN F1:{:.4f}, STD F1:{:.4f}, MAX F1:{:.4f}\n"
                .format(mean_f1,std_f1,max_f1))
        f.write("MEAN PRES:{:.4f}, STD PRES:{:.4f}, MAX PRES:{:.4f}\n"
                .format(mean_pres,std_pres,max_pres))
        f.write("MEAN RECALL:{:.4f}, STD RECALL:{:.4f}, MAX RECALL:{:.4f}"
                .format(mean_recall,std_recall,max_recall))
